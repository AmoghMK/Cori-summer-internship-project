...
import multivariate_regression as mvr
...
def predict3(x_val,y,x1,x2):
    ...
    theta, cost, it = mvr.regression2(x1, x2, y)

    x_val[0]=(x_val[0]-meanx1)/stddevx1
    x_val[1]=(x_val[1]-meanx2)/stddevx2
    y_val=mvr.predict(x_val[0],x_val[1],theta)
    y_val=np.array(y_val)*stddevy+meany
    return(y_val)
